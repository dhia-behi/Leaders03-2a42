<?php
include('C:\xampp\htdocs\web\projet\projet\controller\EvenementsController.php');
include('C:\xampp\htdocs\web\projet\projet\model\Evenements.php');
$actC = new evenementsController();
if( isset($_POST['idformation']) && isset($_POST['titre']) && isset($_POST['description']) && isset($_POST['heure_debut']) && isset($_POST['heure_fin']) && isset($_POST['place']) && isset($_POST['image'])){
    $evenements = new evenements($_POST['titre'],$_POST['description'],$_POST['heure_debut'],$_POST['heure_fin'],$_POST['place'],$_POST['image'],$_POST['idformation']);
    $actC->addevenements($evenements);
    header('Location: evenementsBack.php?idformation='.$_POST['idformation']);
}
$evenements = $actC->joinformation($_GET['idformation']);
?>
<!DOCTYPE html>
<html lang="en">
  <head>
    ...
    <link rel="stylesheet" href="view\backOffice\assets\vendors\css\vendor.bundle.base.css">
    <!-- endinject -->
              ...
              <div class="col-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                    ...
                    <form id="myForm" class="forms-sample" action="evenementsBack.php" method="post">
                      <input type="text" name="idformation" value="<?php echo $_GET['idformation']; ?>" hidden>
                    ...
                    <div class="table-responsive">
                      <table class="table">
                        ...
                        <tbody>
                          ...
                          <tr>
                            <td><?php echo $evenements['idevenements']; ?></td>
                            <td><?php echo $evenements['titre']; ?></td>
                            <td><?php echo $evenements['description']; ?></td>
                            <td><?php echo $evenements['heure_debut']; ?></td>
                            <td><?php echo $evenements['heure_fin']; ?></td>
                            <td><?php echo $evenements['place']; ?></td>
                            <td><img src="../images/<?php echo $evenements['image']; ?>" style="width: 60px; height: 60px;" ></td>
                            <td><?php echo $evenements['ctitre']; ?></td>
                            <td><a class="btn btn-primary mr-2" href="editEvenements.php?id=<?php echo $evenements['idevenements']; ?>">Modifier</a> <button class="btn btn-primary mr-2" onclick="confirmDelete(<?php echo $evenements['idevenements']; ?>, <?php echo $_GET['idformation']; ?>,'deleteEvenements.php')">Supprimer</button></td>
                          </tr>
                          <?php }} ?>
                        </tbody>
                      </table>
                      <script>
                        function confirmDelete(articleId, idformation, redirectUrl) {
                        var confirmation = confirm("Êtes-vous sûr de vouloir supprimer cet activité ?");
                        if (confirmation) {
                            window.location.href = redirectUrl + "?id=" + articleId + "&idformation=" + idformation;
                        }
                      } 
                      </script>
                    </div>
